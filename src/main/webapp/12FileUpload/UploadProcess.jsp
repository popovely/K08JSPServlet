...
try {
	...
	*/
	...
	/* 
		파일명이 한글인 경우 서버에서 문제를 일으킬 수 있으므로
		영문과 숫자로 변경하는 것이 좋다.
		아래에서는 현재 날짜와 시간을 통해 생성된 문자열을 파일명으로 설정한다.
	*/
	// mr객체를 통해 서버에 저장된 파일명을 가져온다.
	String fileName = mr.getFilesystemName("attachedFile");
	// 파일명에서 .의 위치를 파악한 후 확장자를 따낸다.
	String ext = fileName.substring(fileName.lastIndexOf("."));
	// 현재날짜와 시간(밀리세컨즈까지)을 이용해서 파일명으로 사용할 문자열을 만든다.
	String now = new SimpleDateFormat("yyyyMMdd_HmsS").format(new Date());
	// 확장자와 파일명을 합쳐서 저장할 파일명을 만들어준다.
	String newFileName = now + ext;
	
	// 기존의 파일명을 새로운 파일명으로 변경한다.
	// File.separator : (운영체제마다 다른) 경로구분 기호를 붙여준다. Window는 /
	...
	/* 
	파일을 제외한 나머지 폼값을 받는다.
	이때 request객체를 통해 받는것이 아니므로 주의해야 한다.
	*/
