...
<body>
	...
	<!-- 
		영역에 저장된 속성에 EL로 접근할 때는
		속성명이 중복되지 않는 경우라면 EL내장객체 없이 속성명만으로 접근가능
			; requestScope.personObj.name 형태로 기술하지 않아도 된다.
	 -->
	<ul>
		<!-- 
			"속성명.멤버변수" 형태로 기술하면
			객체의 getter()를 자동으로 호출하게 되어 값이 출력된다.
				; 아래 문장은 getter()를 호출하므로
	...
	// 따라서 영역에서 객체를 꺼내서 사용할 때는 원래의 형으로 형변환 해야한다.
	...
	<ul>
		<!-- 
			EL에서는 산술연산이 가능하므로 아래에서 30이 출력된다.
