...
<body>
	<!-- 
		폼값이나 파라미터를 받을때 EL의 내장객체를 통해 사용할 수 있다.
		
		param
			: 단일값을 받을 때 사용한다.
		paramValues
			: 2개 이상의 값을 받을 때 사용한다.
			  이 경우에는 배열을 통해 값을 구분해야 한다.
	 -->
	<h3>EL로 폼값 받기</h3>
	<ul>
		<li>이름 : ${ param.name }</li>
		...
		<!-- 
			...
			EL은 NullPointerException이 발생하지 않으므로 위와 같이 기술해도 문제없다.
		 -->
	</ul>
	...
	/* 
		JSP 내장객체를 통해 폼값을 받은 후 출력하는 경우
		선택된 값이 1개밖에 없다면 에러가 발생하게 된다.
		이 경우 if문으로 값이 있는지 체크하거나 예외처리를 해야하므로
		EL에 비해서는 불편한 점이 있다.
	*/
	%>
</body>
