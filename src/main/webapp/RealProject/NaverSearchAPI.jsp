...
<script>
/* 
	VSCode에서 K07JQuery폴더에 02Ajax 참고
*/
$(function(){
	// $.ajax메소드에서 반복적으로 사용되는 속성을 묶어 정의한다.
	$.ajaxSetup({
		...
		error : errFunc 		// 성공했을때 호출할 콜백메소드
	});
	/* 
		앞의 $.ajaxSetup() 메소드에서
		모든 속성에 대해 설정이 완료되었으므로
		아래에서는 추가로 설정할 값이 없다.
		이 경우에는 속성을 생략해도 무방함.
	*/
...
function sucFuncJson(d){
	...
	*/
	...
	/*
	$.each(배열데이터, function(반복시 index반환, 반복시 해당객체 반환){}
		: 반복된 요소들을 하나의 문자열로 만들어준다.
	*/
...
<div class="container">
	<!-- 
		Ajax를 통해 비동기 통신을 구현할 때는
		반드시 요청한 페이지에서 콜백할 데이터가 정상적으로 출력되는지
		직접 확인해야 한다. (매우 중요)
	 -->
	<div class="row">
