...
<head>
    ...
    <script>
        // 매개변수를 통해 form태그의 DOM객체를 사용할수 있다.
        var isValidate = function(frm){
            ...
            // 비밀번호 : 비번과 비번확인의 입력값이 동일한지 검증. 다르면 재입력 요구
            if(frm.pwd.value != frm.pwd2.value){
                alert('입력한 패스워드가 일치하지 않습니다.');
                frm.pwd.value="";    //입력되어있는 패스워드를 지운다1
                frm.pwd2.value="";   //입력되어있는 패스워드를 지운다2
                frm.pwd.focus();
                return false;
            }
            ...
            */
            var isGender = false;
            // 성별에 대한 라디오 항목의 개수만큼 반복. DOM을 통해 개수를 알 수 있다.
            for(var i=0; i<frm.gender.length; i++){
                // 체크된 항목이 있는지 확인
                if(frm.gender[i].checked==true){
                    // 있다면 true로 값 변경 후 루프탈출
                    isGender = true;
                    // 라디오는 항목 하나만 선택가능하므로 더이상 확인할 이유가 없다.
                    break;
                }
            }
            // 성별을 체크하지 않았다면 경고창을 띄움
            if(isGender != true){
                alert("성별을 선택해 주세요");
            ...
            /*
            checkbox는 여러개를 선택할 수 있으므로
            체크된 항목을 카운트해서 검증해야 한다.
            */
            var checkCount = 0;
            ...
            */
            ...
            /*
            textarea는 자체적인 value속성은 없으나
            JS에서 DOM객체를 통한 검증을 할때에는
            value속성을 사용해야 한다.
            */
            if(frm.intro.value==""){
                alert("자기소개를 입력하세요");
        ...
        function idCheck(fn){
            ...
            if(fn.id.value==""){
                ...
                readonly속성을 추가한다.
                만약 부모창에서 수정이 가능하면
                사용자가 중복확인 이후에 수정할 수도 있으므로 주의해야 한다.
                */
                fn.id.readOnly = true;
                /*
                부모창에서 입력된 아이디를 파라미터로 팝업창에 전달한다.
    ...
    <table border="1" cellspacing="0" cellpadding="5">
        <tr>
            ...
            <td>
                ...
                <!-- 
                    type="button"을 기술하지 않으면 submit으로 인식될 수 있음.
                    ; 빼먹지말고 써라
                 -->
