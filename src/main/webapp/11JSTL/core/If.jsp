...
<body>
	...
	<!-- 
		JSTL의 if태그는 else 구문이 따로 없으므로
		두개의 if태그를 통해 else와 같이 사용해야 한다.
	...
	<h4>조건식 주의사항</h4>
	<!-- 
		결과가 false이면 태그 사이의 내용도 출력되지 않는다.
	 -->
	...
	<c:if test="${ not empty param.user and not empty param.pass }">
		...
		<!-- 
		 ...
		 -->
		<c:if test="${ param.user=='kosmo' && param.pass eq '1234' }" var="result">
			${ param.user }님 하이룽!
		</c:if>
	...
	<c:if test="${ not(empty id or empty pwd) }">
		<c:if test="${ id eq 'kosmo' and pwd eq 1234 }" var="result">
			kosmo님 하이룽~
		</c:if>
		<c:if test="${ not result }">
			아이디와 비번을 확인하세요
