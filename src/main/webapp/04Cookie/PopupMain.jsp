...
if(cookies != null){
	for(Cookie c : cookies){
		...
		// 쿠키명이 PopupClose 일 때 쿠키값을 변수에 저장
...
$(function(){
	...
	$('#closeBtn').click(function(){
		...
		$.ajax({
			...
			success : function(resData){// 성공했을때의 콜백메소드
...
<%
	for(int i=1; i<=10; i++){
		/* 
		println()을 사용하면 웹 페이지에서는 줄바꿈이 되지 않고
		공백으로 취급되기 때문에 줄바꿈을 위해서는 <br>태그를 따로
		사용해야 한다.
		다만 웹 페이지에서 우클릭>소스보기를 했을때에는
		해당 내용이 줄바꿈된 형태로 보이기 때문에 더 보기에 좋긴함.
		*/
