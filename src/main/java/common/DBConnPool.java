...
public class DBConnPool {
		...
		try {
			...
			/*
			   2. 앞에서 생성한 객체를 통해 JNDI서비스 구조의 초기 Root디렉토리를 얻어온다.
			      여기서 얻어오는 톰캣의 루트 디렉토리명은
			      java:comp/env로 이미 정해져 있으므로 그대로 사용하면 된다
			 */
			Context ctx = (Context)initCtx.lookup("java:comp/env");
			/*
			   3. server.xml에 등록된 네이밍을 lookup해서 DataSource를 얻어온다.
	...
	/* 
	사용이 끝난 객체를 Pool에 반납한다.
		; 여기서의 close()는 객체의 소멸이 아니다.
	 */
	public void close() {
		try {
			if (rs != null) rs.close();
