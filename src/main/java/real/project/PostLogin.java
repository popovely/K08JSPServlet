...
public class PostLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
			JSP에서 JSON을 생성하기 위해서는
			확장 라이브러리가 추가적으로 필요하다.
			json-simple-1.1.1.jar 파일을 다운로드 한 후 lib폴더에 추가해야 함.
		 */
		// JSON객체를 생성하기 위한 객체
		JSONObject json = new JSONObject();
		
		// DTO객체를 통해 회원의 이름이 있는지 확인해서...
		if (dto.getName() != null) {// 있으면 로그인 성공
			json.put("result", 1);
			json.put("message", dto.getName() +"님, 로그인 성공★");
